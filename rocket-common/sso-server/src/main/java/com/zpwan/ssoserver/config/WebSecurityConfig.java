...
public class WebSecurityConfig  extends WebSecurityConfigurerAdapter{
    ...
    @Autowired
    public WebSecurityConfig(@Qualifier("JwtAuthenticationEntryPoint")
            JwtAuthenticationEntryPoint unauthorizedHandler,
                             @Qualifier("RestAuthenticationAccessDeniedHandler") AccessDeniedHandler accessDeniedHandler,
                             @Qualifier("CustomUserDetailsService") UserDetailsService customUserDetailsService,
                             @Qualifier("authenticationTokenFilter")JwtAuthenticationTokenFilter authenticationTokenFilter) {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        httpSecurity
                ...
                .antMatchers("/sso/user/login").permitAll()
                // 除上面外的所有请求全部需要鉴权认证
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/v2/api-docs","/sso/user/login",
